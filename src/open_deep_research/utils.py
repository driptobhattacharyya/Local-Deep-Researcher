...
async def tavily_search(
    ...
    summarization_model = init_chat_model(
        ...
        **get_model_kwargs(configurable.summarization_model, configurable.summarization_model_max_tokens),
...
def get_model_kwargs(model_name: str, max_tokens: Optional[int] = None) -> dict[str, Any]:
    """Return provider-specific kwargs for init_chat_model/with_config.

    For Ollama models, map max_tokens -> num_predict. For others, keep max_tokens.
    """
    kwargs: dict[str, Any] = {}
    if max_tokens is not None:
        # Normalize to lowercase for provider detection
        model_l = (model_name or "").lower()
        if model_l.startswith("ollama:") or model_l == "ollama":
