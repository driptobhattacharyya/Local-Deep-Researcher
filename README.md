...
You can run the agent fully locally with [Ollama](https://ollama.com/). Steps:

1) Install Ollama and start the service.

2) Pull a model, e.g. llama3.1 or qwen2.5:
...
- We automatically map `max_tokens` to Ollama's `num_predict` under the hood, so your token settings still apply.
- If the model supports tool-calling/JSON output, those features are used as in other providers.
- Make sure the Ollama server is running locally (default http://127.0.0.1:11434).
